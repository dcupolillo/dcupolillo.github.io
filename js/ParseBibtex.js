class ParseBibtex {
    ...
    get author() {
        let reg = /author *= *\{(.*?)\}/;
        if (this._source.match(reg) != null) {
            let rawAuthors = this._source.match(reg)[1].toString();
    
            // Split authors by " and " and format each
            return rawAuthors
                .split(" and ")
                .map(author => {
                    let parts = author.split(",").map(part => part.trim()); // Split by comma and trim
                    let surname = parts[0]; // Surname is the first part
                    let givenNames = parts[1]?.split(" ") || []; // Split given names
                    let initials = givenNames.map(name => name[0] + ".").join(" "); // Convert to initials
                    let formattedAuthor = `${surname} ${initials}`;
                    if (formattedAuthor === "Cupolillo D.") {
                        return `<strong>${formattedAuthor}</strong>`;
                    }
                    return formattedAuthor;
                })
                .join(", "); // Join all formatted authors with commas
        } else {
            return null;
        }
    }
    ...
    get title() {
        let reg = /title *= *\{(.*?)\}/;
        if (this._source.match(reg) != null) {
            let title = this._source.match(reg)[1].toString();
